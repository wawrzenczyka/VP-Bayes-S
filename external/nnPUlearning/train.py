...
from model import CNN, LinearClassifier, MultiLayerPerceptron, ThreeLayerPerceptron
...
def process_args(arguments):
    ...
    args = parser.parse_args(["--preset", "exp-mnist"])
...
def main(arguments):
    ...
    valid_iter = chainer.iterators.SerialIterator(
        XYtrain, args.batchsize, repeat=False, shuffle=False
    )
    test_iter = chainer.iterators.SerialIterator(
        XYtest, args.batchsize, repeat=False, shuffle=False
    )
    ...
    loss_funcs = {
        "nnPU": PULoss(
            prior, loss=loss_type, nnpu=True, gamma=args.gamma, beta=args.beta
        ),
    ...
    updater = MultiUpdater(
        train_iter, optimizers, models, device=args.gpu, loss_func=loss_funcs
    )
    trainer = chainer.training.Trainer(updater, (args.epoch, "epoch"), out=args.out)
    trainer.extend(extensions.LogReport(trigger=(1, "epoch")))
    train_01_loss_evaluator = MultiPUEvaluator(
        prior, valid_iter, models, device=args.gpu
    )
    ...
    if extensions.PlotReport.available():
        trainer.extend(
            extensions.PlotReport(
                ["train/nnPU/error", "train/uPU/error"],
                "epoch",
                file_name="training_error.png",
            )
        )
        trainer.extend(
            extensions.PlotReport(
                ["test/nnPU/error", "test/uPU/error"],
                "epoch",
                file_name="test_error.png",
            )
        )
