...
def get_mnist():
    ...
    x = np.reshape(x.to_numpy(), (x.shape[0], 1, 28, 28)) / 255.0
...
def make_dataset(dataset, n_labeled, n_unlabeled):
    def make_pu_dataset_from_binary_dataset(
        x, y, labeled=n_labeled, unlabeled=n_unlabeled
    ):
        labels = np.unique(y)
...
def load_dataset(dataset_name, n_labeled, n_unlabeled):
    ...
    xy_train, xy_test, prior = make_dataset(
        ((x_train, y_train), (x_test, y_test)), n_labeled, n_unlabeled
    )
