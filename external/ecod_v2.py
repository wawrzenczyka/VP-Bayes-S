...
class ECODv2(BaseDetector):
    ...
    def decision_function(self, X):
        ...
        if (
            not hasattr(self, "ecdfs_l")
            or not hasattr(self, "ecdfs_r")
            or not hasattr(self, "skewness")
        ):
            raise NotFittedError(
                "This ECOD instance is not fitted yet. Call 'fit' with appropriate arguments before using this estimator."
            )

        self.U_l = -1 * np.log(_calculate_ecdf(self.ecdfs_l, X))
        self.U_r = -1 * np.log(_calculate_ecdf(self.ecdfs_r, -X))

        self.U_skew = self.U_l * -1 * np.sign(self.skewness - 1) + self.U_r * np.sign(
            self.skewness + 1
        )
