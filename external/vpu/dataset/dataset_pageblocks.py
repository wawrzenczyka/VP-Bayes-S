...
def get_pageblocks_loaders(batch_size=500, num_labeled=2000, positive_label_list=[1], test_proportion=0.4,
                        if_normalized=False):
    val_x_size = 500
    data_frame = pd.read_csv(os.getcwd() + '/data/pageblocks.txt', sep=',', header=None)
    ...
    for data, target in test_set_unprocessed:
        if target in positive_label_list:
            ...
            test_positive_set.append((data, target))
        else:
            target = 0
            test_set.append((data, target))
