...
def get_checkpoint_path(config):
    ...
    checkpoint_path = os.path.join(
        os.getcwd(),
        config.dataset,
        "P=" + str(config.positive_label_list),
        "lr=" + str(config.learning_rate),
        "lambda=" + str(config.lam),
...
def save_checkpoint(
    state, is_lowest_on_val, is_highest_on_test, config, filename="checkpoint.pth.tar"
):
    """
    ...
    if is_lowest_on_val:
        shutil.copyfile(
            filepath, os.path.join(checkpoint, "model_lowest_on_val.pth.tar")
        )
    if is_highest_on_test:
        shutil.copyfile(
            filepath, os.path.join(checkpoint, "model_highest_on_test.pth.tar")
        )
